...
2a. **Web fetch fallback protocol (required):**
    - If `WebFetch` fails with domain safety verification or network policy errors, do **not** retry the same blocked URL repeatedly.
    - Immediately switch to `WebSearch` and gather content from alternative sources:
       - publisher/working-paper pages (NBER, CEPR, IZA, SSRN, RePEc)
       - preprint mirrors (arXiv/bioRxiv where relevant)
       - DOI landing pages and abstract pages
       - course notes or GitHub mirrors that quote the same material
    - If direct fetch still fails, use `Bash` with `curl -L` as a fallback only for public, non-authenticated pages.
    - If all external fetching is blocked, ask the user to provide one of:
       - pasted excerpt
       - local PDF/HTML/TXT in workspace
       - screenshot + key text
    - Continue the review with available evidence and explicitly mark unverified claims.
...
- **Treat fetch failures as recoverable.** Continue with fallback sources instead of stopping the workflow.
